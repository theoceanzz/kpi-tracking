...
type Kind = 'perspective' | 'objective' | 'kr' | 'kpi'
...
function buildGraph(map: StrategyMapResponse): { nodes: Node[]; edges: Edge[] } {
  ...
  map.perspectives.forEach(lane => {
    ...
    nodes.push({ id: pid, type: 'strategy', position: { x: 0, y: 0 }, data: { label: lane.name, kind: 'perspective', color: lane.color } })
    lane.objectives.forEach(obj => {
      const oid = `o:${obj.id}`
      objKind.set(obj.id, true)
      nodes.push({ id: oid, type: 'strategy', position: { x: 0, y: 0 }, data: { label: obj.name, sub: obj.code, kind: 'objective', color: lane.color } })
      edges.push({ id: `${pid}-${oid}`, source: pid, target: oid, data: { hierarchy: true }, style: { stroke: lane.color, strokeWidth: 1.5 } })
      obj.keyResults.forEach(kr => {
        const kid = `k:${kr.id}`
        nodes.push({ id: kid, type: 'strategy', position: { x: 0, y: 0 }, data: { label: kr.name, sub: 'KR', kind: 'kr', color: lane.color } })
        edges.push({ id: `${oid}-${kid}`, source: oid, target: kid, data: { hierarchy: true }, style: { stroke: '#cbd5e1', strokeWidth: 1.5 } })
        kr.kpis.forEach(kpi => {
          const kpid = `kpi:${kpi.id}`
          nodes.push({ id: kpid, type: 'strategy', position: { x: 0, y: 0 }, data: { label: kpi.name, sub: 'KPI', kind: 'kpi', color: lane.color, mismatch: kpi.perspectiveMismatch } })
          edges.push({ id: `${kid}-${kpid}`, source: kid, target: kpid, data: { hierarchy: true }, style: { stroke: '#e2e8f0', strokeWidth: 1.5 } })
        })
      })
    })
    // KPI trực tiếp dưới viễn cảnh (không qua OKR)
    map.directKpis.filter(k => k.perspectiveId === lane.perspectiveId).forEach(kpi => {
      const kpid = `kpi:${kpi.id}`
      nodes.push({ id: kpid, type: 'strategy', position: { x: 0, y: 0 }, data: { label: kpi.name, sub: 'KPI trực tiếp', kind: 'kpi', color: lane.color } })
      edges.push({ id: `${pid}-${kpid}`, source: pid, target: kpid, data: { hierarchy: true }, style: { stroke: lane.color, strokeWidth: 1.5, strokeDasharray: '4 3' } })
    })
  })
...
export default function BscStrategyMapPage() {
  ...
  // Objective CHƯA gán viễn cảnh bị BscStrategyMapService loại khỏi bản đồ (không thuộc làn nào).
  // Đếm riêng để nói đúng việc cần làm là "gán viễn cảnh", không phải "tạo thêm Mục tiêu".
    ...
    <div className="max-w-full h-full flex flex-col gap-4">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 shrink-0">
        <div>
          ...
          <p className="text-slate-500 font-medium mt-1">Viễn cảnh → Mục tiêu → Kết quả then chốt → KPI, và quan hệ nhân-quả giữa các mục tiêu</p>
        </div>
        <div className="flex items-center gap-3 text-[11px] font-bold text-slate-500">
          ...
          <span className="flex items-center gap-1.5"><AlertTriangle size={12} className="text-amber-500" /> Lệch viễn cảnh</span>
        </div>
      </div>
      ...
      {!isLoading && (objectiveCount < 2 ? (
        ...
        <div className="shrink-0 flex items-start gap-2.5 rounded-2xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-900/10 px-4 py-2.5">
          ...
          <p className="text-[11px] leading-relaxed text-amber-800 dark:text-amber-300">
            ...
            {unassignedCount > 0 ? (
              <> Bạn đang có <b className="font-black">{unassignedCount} Mục tiêu chưa gán viễn cảnh</b> nên <b className="font-black">không hiện ở đây</b>. Vào <b className="font-black">Quản lý OKR</b> → sửa Mục tiêu → chọn <b className="font-black">Viễn cảnh BSC</b>, chúng sẽ xuất hiện ngay.</>
            ) : (
              <> Hãy tạo thêm Mục tiêu ở trang <b className="font-black">Quản lý OKR</b> và nhớ gán <b className="font-black">Viễn cảnh BSC</b> — chưa gán viễn cảnh thì Mục tiêu không lên bản đồ.</>
            )}
          </p>
        </div>
      ) : (
        <div className="shrink-0 flex items-start gap-2.5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/50 px-4 py-2.5">
          ...
          <p className="text-[11px] leading-relaxed text-slate-600 dark:text-slate-300">
            ...
            {unassignedCount > 0 && (
              <> <span className="text-amber-600 dark:text-amber-400">Còn <b className="font-black">{unassignedCount} Mục tiêu chưa gán viễn cảnh</b> nên chưa hiện trên bản đồ — gán viễn cảnh ở Quản lý OKR để đưa lên.</span></>
            )}
          </p>
        </div>
      ))}
