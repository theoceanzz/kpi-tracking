...
const TREND_PERIODS = [
  { label: '5 đợt gần nhất', value: 'LAST_5' },
  { label: '10 đợt gần nhất', value: 'LAST_10' },
  { label: '15 đợt gần nhất', value: 'LAST_15' },
];
...
export default function SummaryTab() {
  ...
  const periodOptions = useMemo(() => {
    if (!kpiPeriodsData?.content) return [{ label: 'Tất cả', value: 'ALL' }]
    return [
      { label: 'Tất cả', value: 'ALL' },
      ...kpiPeriodsData.content.map(p => ({ label: p.name, value: p.id }))
    ]
  }, [kpiPeriodsData])
  ...
  const handleTogglePin = async (widget: SummaryWidget) => {
    ...
    if (!targetWidgetId) {
      // If user hasn't saved dashboard config yet, we need to initialize it first
      try {
        ...
        return; // saveConfig already shows error toast
      }
    }
