...
import { Loader2, X, AlertTriangle, Info, ShieldCheck, Award, ExternalLink } from 'lucide-react'
      ...
      <div className="space-y-5">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            />
            {errors.points && <p className="mt-1 text-xs text-[var(--color-error)]">{errors.points.message}</p>}
          </div>
        ...
        */}
        ...
        {notice && (
          <div
            className={
              notice.tone === 'warn'
                ? 'flex items-start gap-2 rounded-card border border-[var(--color-warning-border)] bg-[var(--color-warning-bg)] px-4 py-3 text-sm'
                : 'flex items-start gap-2 rounded-card border border-[var(--color-info-border)] bg-[var(--color-info-bg)] px-4 py-3 text-sm'
            }
          >
            {notice.tone === 'warn' ? (
              <AlertTriangle size={16} className="mt-0.5 flex-shrink-0 text-[var(--color-warning)]" />
            ) : (
              <Info size={16} className="mt-0.5 flex-shrink-0 text-[var(--color-info)]" />
            )}
