    ...
    <div>
      ...
      <div className="bg-[var(--color-card)] rounded-xl border border-[var(--color-border)] p-6 max-w-2xl space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4 text-sm">
          <div><p className="text-xs text-[var(--color-muted-foreground)]">Giá trị thực tế</p><p className="font-medium">{submission.actualValue}</p></div>
          <div><p className="text-xs text-[var(--color-muted-foreground)]">Mục tiêu</p><p className="font-medium">{submission.targetValue ?? '—'}</p></div>
