  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit((data) => mutation.mutate(data))} className="bg-[var(--color-card)] rounded-card border border-[var(--color-border)] p-6 max-w-md space-y-4">
        <div>
          <label className="text-label block font-medium mb-1.5">Mật khẩu hiện tại</label>
        ...
        <div>
          <label className="text-label block font-medium mb-1.5">Mật khẩu mới</label>
          ...
          {watch('newPassword') && watch('currentPassword') && watch('newPassword') === watch('currentPassword') && (
            <p className="text-[var(--color-error)] text-xs mt-1">Mật khẩu mới không được trùng với mật khẩu cũ</p>
          )}
        </div>

        <div>
          <label className="text-label block font-medium mb-1.5">Xác nhận mật khẩu mới</label>
