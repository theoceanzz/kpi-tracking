...
export interface TreemapLeaf {
  ...
  assignees?: { userId: string; fullName: string; avatarUrl?: string | null }[]
}
 ...
 */
function AssigneeAvatar({ name, url }: { name: string; url?: string | null }) {
    ...
    <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-3.5 rounded-xl shadow-lg">
      ...
      {people.length > 0 && (
        <div className="flex items-center gap-2 pt-2.5 mt-2.5 border-t border-slate-100 dark:border-slate-800">
          <span className="text-xs text-slate-500 font-medium shrink-0">Đảm nhiệm:</span>
          ...
          {/* Từ hai người trở lên chỉ còn avatar: liệt kê tên sẽ làm tooltip cao quá, che mất
              chính biểu đồ ở dưới. Trỏ chuột vào từng avatar vẫn xem được tên. */}
          {people.length === 1 && (
