...
### R16. Bút chì "ô này sửa được" — `src/index.css` (rule toàn cục) [chốt 18/09]
Nhân viên phản hồi không phân biệt được ô nhập với chữ tĩnh (nhất là ô nền muted, ô sửa tại chỗ không viền). Mọi `<input>` chữ/số/email/mật khẩu và `<textarea>` tự có bút chì mờ ở mép phải (textarea: góc trên phải) qua `background-image`, không cần bọc thẻ; ẩn khi focus / disabled / readOnly, không vẽ cho `type=search`, placeholder "Tìm…", date/file/color/range/checkbox. Rule nằm ngoài `@layer` nên đè `padding-right` của Tailwind: **ô nào đặt nút hoặc đơn vị ở mép phải (mắt mật khẩu, "%", đơn vị KPI, nút xoá, mic/gửi) phải thêm class `no-edit-hint`** — nếu không thì bút chì đè lên nút và `pr-*` bị ghi đè.

### R17. `Input` / `Textarea` — `src/components/ui/input.tsx`, `textarea.tsx` [chốt 18/09 tại `UserFormModal`]
Ô nhập chuẩn, cùng thang 32/36/40 (`size="sm" | "default" | "lg"`) và cùng viền `--color-input` / focus ring với `Button`, `SelectTrigger`. Props: `prefix` (icon trước), `suffix` (đơn vị, mắt mật khẩu, nút xoá — tự tắt bút chì R16 và không phải đoán `pr-*`), `invalid={!!errors.x}` (viền đỏ + `aria-invalid`), `inputClassName` (class cho `<input>` trong khi có prefix/suffix). Không có prefix/suffix thì render đúng một thẻ `<input>` nên thay thế `<input>` thô không đổi bố cục. `Textarea` mặc định 3 dòng, `resize-y`. **Code mới không viết `<input className="h-9 w-full rounded-control …">` tay nữa**; `const inputCls = …` trong trang là dấu hiệu cần chuyển. Ô cố ý khác (sửa tại chỗ trong bảng, ô số hẹp, composer chat) được để `<input>` thô. Số còn lại theo dõi ở §T.

## S. Nhật ký quyết định theo lượt

| Ngày | Trang | Pattern chạm | Quyết định mới / thay đổi |
|---|---|---|---|
| 2026-09-18 | Form người dùng (`UserFormModal`) | R17 | Tạo `Input`/`Textarea` dùng chung; chuyển form tạo/sửa người dùng làm mẫu (ô mật khẩu 2 nút → `suffix`, bỏ `inputCls` cả ở `SelectTrigger` để cùng chiều cao 36px). Codemod các ô còn lại để lượt riêng có ảnh chụp |
| 2026-09-18 | Toàn bộ ô nhập (~400 `<input>`/`<textarea>`) | R16 | Thêm bút chì mờ báo "ô này sửa được" bằng một rule CSS toàn cục thay vì sửa từng ô; opt-out `no-edit-hint` đã gắn cho 16 ô có widget bên phải (5 trang mật khẩu, hồ sơ, form người dùng, điều chỉnh KPI, xem trước Excel, nộp báo cáo có đơn vị, ví, chọn ngân hàng, hai ô soạn K.AI) |
...
| 2026-09-18 | Minh chứng đính kèm cho lượt chấm (`features/evidence`) | P4, R16 | Khối `EvidenceAttachments` dùng chung (tiêu đề + đếm N/5 + gợi ý định dạng + nút "Đính kèm tệp"; danh sách tệp có icon theo loại, cỡ, mở tab mới, xoá qua ConfirmDialog). Tệp gắn vào KHOÁ (đợt/kỳ/phạm vi + người) chứ không vào bản ghi điểm, nên đính kèm được trước khi bấm lưu và nhân viên – quản lý cùng nhìn một danh sách. Đặt ở: chấm đợt (`StaffEvaluationModal`), chốt kỳ (`UserScoreModal`), tự đánh giá (`EvaluationFormModal`), phiếu hạnh kiểm (`ConductSheetTable`). Backend: bảng `evidence_attachments` (V10), `GET/POST /evidence`, `DELETE /evidence/{id}` |
| 2026-09-17 | **OKR của tôi**, **BSC của tôi** (mới, trong "Của tôi") | P2 | Cùng khuôn KPI của tôi: WorkspaceHeader 4 số liệu (…, Cần nộp) · FilterBar · danh sách nhóm. OKR: mục tiêu → kết quả then chốt (tiến độ) → KPI của tôi; phạm vi Tôi tham gia / Đơn vị tôi / Toàn công ty. BSC: bộ tiêu chí áp cho đơn vị tôi theo đợt (kế thừa cấp trên), hạng mục gom theo 4 lĩnh vực → KPI của tôi; nhãn % trọng số đã gắn BSC. Dòng KPI trong nhóm = `MyKpiMiniRow` (tên · tiến độ nộp · hạn · Nộp bài/Đã xong/Quá hạn), trạng thái tính bằng `kpiWorkState` dùng chung |
| 2026-09-17 | Xếp loại đơn vị (`UnitClassificationConfigSection`, `BellCurveEditor`) | P3 | Accordion xếp dọc → hai cột: danh sách hồ sơ trái (tên, phạm vi, cảnh báo chưa gán) · bộ soạn phải chia 3 khối "Áp dụng cho / Luật xếp loại / Khung bell curve". Luật viết thành câu đọc được ("Đơn vị được xếp Loại 5 khi ≥ 50% nhân sự đạt Loại 5 trở lên"); thêm khối **Thử nhanh** gõ số người mỗi mức → luật khớp được đánh dấu (thuật toán chép từ backend). Bell curve: công tắc ở tiêu đề khối, mẫu phân bố là ChoiceChip, **kéo một mức thì các mức khác tự cân cho đủ 100%** (bỏ trạng thái "tổng 97%"), hàng mức nói thẳng "tối đa N / M người". Xoá hồ sơ qua ConfirmDialog; header báo "Có thay đổi chưa lưu" |
| 2026-09-17 | K.AI (`AiAssistantPage`, `ConversationSidebar`) | P7 | Cột hội thoại: nút "Cuộc trò chuyện mới" nền ai-soft + thu gọn, ô tìm 44px, danh sách gom Đã ghim / Hôm nay / 7 ngày qua / Cũ hơn, menu "⋮" (Ghim · Đổi tên tại chỗ · Xoá); xoá và đổi tên **không hỏi xác nhận** mà cho Hoàn tác trên toast tối (nút violet) — backend có `PATCH /ai/conversations/{id}` và `POST …/restore`, cột `pinned_at` (V9). Màn hình trống: lời chào theo tên + 4 gợi ý dạng viên; header avatar ai-solid + chấm xanh; ô soạn: kẹp tệp trái, mic + gửi phải |
...
|---|---|---|
| `<input>`/`<textarea>` thô chưa qua `Input`/`Textarea` (R17) | ~395 | codemod theo lượt cho ~60% khớp 3–4 công thức chuẩn (form modal, xác thực, dashboard), có ảnh chụp từng trang vì đổi class không được TS bắt; phần cố ý khác giữ nguyên |
