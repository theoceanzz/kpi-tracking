...
/**
    ...
    private String run(String answer, boolean enabled, boolean hasMemory) {
        ...
        return new AnswerValidator(enabled).check(turn, answer);
    }
    ...
    void nullAnswerIsSafe() {
        ...
        turn.setAgentState(st);
        toolsRan.forEach(st::recordSuccess);
        assertThat(new AnswerValidator(true).check(turn, null)).isNull();
    }
    ...
    void neverBlocksWhileAFormIsOpen() {
        ...
        assertThat(new AnswerValidator(true).check(turn, answer)).isEqualTo(answer);
    }
    ...
    void stillBlocksWhenNoFormIsOpen() {
        ...
        assertThat(new AnswerValidator(true).check(turn, "Phòng IT đạt 87 điểm."))
                .contains(BLOCKED);
    }
