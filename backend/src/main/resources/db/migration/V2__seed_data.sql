...
WHERE code IN (
    ...
    'BSC:VIEW', 'BSC:MANAGE_UNIT', 'BSC:APPROVE', 'BSC:OVERRIDE_SCORE', 'BSC:MANAGE', 'BSC:PUBLISH_SCORE',
...
WHERE code IN (
    ...
    'ATTACHMENT:UPLOAD',
    'REMINDER:SEND',
    -- BSC: trưởng đơn vị TỰ LẬP được bộ tiêu chí của đơn vị mình (MANAGE_UNIT) — phó đơn vị thì
    -- không: một đơn vị chỉ nên có MỘT người chịu trách nhiệm bộ tiêu chí. OKR vẫn chỉ xem.
...
--
-- Bất biến phải giữ: TỔNG hạn mức cấp từ ngân sách công ty ≤ ngân sách công ty.
-- 30 người mẫu × 1.000.000 = 30.000.000 ≤ 50.000.000, còn dư chỗ cho người tạo thêm.
-- Sửa hai con số này thì phải kiểm lại phép nhân, nếu không AiQuotaAllocationService
-- sẽ chặn mọi lần phân bổ tiếp theo vì túi đã âm.
...
-- Bộ tiêu chí không còn trỏ thẳng tới một đợt: nó mang `apply_scope` và nối với đợt qua bảng
-- `bsc_scorecard_periods`. Ghép theo TÊN vì INSERT ... SELECT không trả về được cột nguồn để nối
-- trực tiếp — cùng cách mục 18.3 ở trên đang dùng.
INSERT INTO bsc_scorecards
    (organization_id, apply_scope, name, vision, status, scoring_mode, empty_perspective_policy)
SELECT p.organization_id, 'PERIOD',
       'Bộ tiêu chí — ' || p.name,
...
SELECT s.id, p.id, ch.old_w, ch.new_w,
       ...
       ch.at::timestamptz
FROM bsc_scorecards s
-- Đợt của bộ tiêu chí nằm ở bảng nối chứ không còn là một cột trên chính nó.
JOIN bsc_scorecard_periods sp ON sp.scorecard_id = s.id
JOIN kpi_periods kp ON kp.id = sp.kpi_period_id
JOIN bsc_perspectives p ON p.organization_id = s.organization_id AND p.deleted_at IS NULL
JOIN (VALUES
